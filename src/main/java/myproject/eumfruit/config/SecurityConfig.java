...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()    // 시큐리티 처리에 HttpServletRequest를 이용한다
                .mvcMatchers("/", "/members/**", "/item/**", "/images/**", "/fonts/**", "/picture/**","/popup/**", "/contact/**", "/blog/**","/cafe/**","band/**").permitAll() // permitAll()을 통해 모든 사용자가 인증(로그인)없이 경로에 접근할 수 있도록 설정한다. 메인페이지, 회원관련url, 상품페이지 경로 등이 이에 해당한다.
