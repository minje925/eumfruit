...
public class SmsService {
    ...
    private String api_key = "NCSE5VLMN8FCBWUQ";
    private String api_secret = "OTCKR851NVAQHRPP9S7XDOHV1KLHRBEP";
    public void sendSms(Order order, SmsType smsType) {

        Message coolsms = new Message(api_key, api_secret);
        HashMap<String, String> params = new HashMap<String, String>();

        if(smsType == SmsType.ORDER) {
        ...
        else if(smsType == SmsType.CANCEL) {
            String cancelDate = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
            String content = "주문 취소 알림!\n" + "취소시각 : " + cancelDate +
                    "\n성명 : " + order.getMember().getName() +
                    "\n상품 : " + order.getOrderItems().get(0).getItem().getItemNm() +
                    "\n수량 : " + order.getOrderItems().get(0).getCount() + "개" +
                    "\n금액 : " + order.getOrderItems().get(0).getTotalPrice() + "원";
